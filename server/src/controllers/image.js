...
export const generateImage = async (req, res) => {
    try {
        ...
        if (user.plan !== "premium") return res.status(403).json({ success: false, message: "Premium feature only" });
        ...
        res.status(500).json({ success: false, message: error.message || "Failed to generate image via Gemini" });
    }
};
...
export const removeImageBackground = async (req, res) => {
    try {
        ...
        if (user.plan !== "premium") return res.status(403).json({ success: false, message: "Premium feature only" });
        if (!image) return res.status(400).json({ success: false, message: "No image file uploaded" });
        ...
        const { secure_url } = await cloudinary.uploader.upload(base64Image, { folder: 'background-removed', resource_type: "image" });

        cleanupFile(image.path);
        await ImageTask.create({ userId, processedImageUrl: secure_url, taskType: 'background_removal' });
...
export const removeImageObject = async (req, res) => {
    try {
        ...
        if (!image || !object) {
            cleanupFile(image?.path);
            return res.status(400).json({ success: false, message: "Image and object required" });
        }

        const { public_id } = await cloudinary.uploader.upload(image.path, { folder: 'object-removed', resource_type: "image" });
        cleanupFile(image.path);

        const imageUrl = cloudinary.url(public_id, { transformation: [{ effect: `gen_remove:${object}` }], resource_type: "image" });
        await ImageTask.create({ userId, processedImageUrl: imageUrl, taskType: 'object_removal' });
